...
describe("BeatController Math Helpers", () => {
  ...
  describe("lerpFx", () => {
    it("should interpolate numerical VFX config options and step non-numerical options", () => {
      const fxA: FxConfig = { wheels: 0.2, clip: "idle", thrust: 0.5 };
      const fxB: FxConfig = { wheels: 0.8, clip: "run", thrust: 1.5 };

      const halfway = lerpFx(fxA, fxB, 0.5);
      expect(halfway.wheels).toBe(0.5);
      expect(halfway.thrust).toBe(1.0);
      // Non-numerical (step animation clip at t >= 0.5)
      expect(halfway.clip).toBe("run");
