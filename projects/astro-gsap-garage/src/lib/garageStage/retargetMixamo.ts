...
/** True for THREE.Bone or Mixamo joint nodes exported without a skin. */
function isJointNode(o: THREE.Object3D): boolean {
  if ((o as THREE.Bone).isBone) return true;
  return /^mixamorig/i.test(o.name);
}
...
/** Clips that break this rig — drop instead of aliasing onto idle (would overwrite Orc Idle). */
export const DROP_CLIP_KEYS = new Set([
  "look",
  "lookingaround",
  "headshake",
  "thoughtfulheadshake",
...
const CANONICAL = new Set([
  "idle", "fight", "repulse", "thrust", "still", "falling", "walk", "run",
  "fireball", "butterflytwirl", "flipkick", "lowcrawl", "standing", "quadpunch",
]);
...
/** True if this Mixamo clip should be ignored for the Mark 85 pack. */
export function shouldDropClip(name: string): boolean {
  return DROP_CLIP_KEYS.has(boneKey(name));
}
...
export function retargetClipsToRoot(
  ...
  for (const clip of sourceClips || []) {
    if (shouldDropClip(clip.name)) continue;
    ...
    // Keep the first clip for a name — later aliases must not overwrite (e.g. look→idle).
    if (!byName.has(key)) byName.set(key, next);
  }
