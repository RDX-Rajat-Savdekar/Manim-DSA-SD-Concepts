...
/**
 * Hotspot locals are model-normalized (× modelSize in CameraRig).
 * Iron Man: pin to chest / face — never reuse camera look targets (those sit mid-body).
 */
const IRON_MAN_HOTSPOT: Record<number, [number, number, number]> = {
  1: [0.0, 0.34, 0.12],   // Arc Reactor (chest triangle)
  2: [0.0, 0.44, 0.1],    // Combat HUD (helmet / face)
  3: [0.0, 0.34, 0.14],   // Unibeam (chest)
  4: [0.08, -0.05, 0.1],  // Wall crawl (low body)
  5: [0.04, 0.15, 0.05],  // Butterfly
  6: [0.0, 0.1, 0.05],    // Hang time
  7: [0.0, 0.05, -0.08],  // Flight
  8: [0.06, 0.2, 0.06],   // Flip kick / nanites
};
...
export function createGarageHud({
  ...
  function buildHotspots() {
    ...
    partBeats.forEach((beat) => {
      ...
      const id = typeof beat.id === "number" ? beat.id : -1;
      const anchor = themeKey === "iron-man" ? IRON_MAN_HOTSPOT[id] : null;
      const local = anchor
        ? new THREE.Vector3(anchor[0], anchor[1], anchor[2])
        : new THREE.Vector3(beat.look.tx, beat.look.ty, beat.look.tz);
      hotspots.push({ el: btn, beat, local });
    });
  }
  ...
  function scrambleHud(beat: Beat | null) {
    ...
    if (scrambleEl) {
      ...
      if (!motionOff) {
        gsap.to(scrambleEl, { duration: 0.45, scrambleText: { text: next, chars: "01■□░▒▓", speed: 0.4 }, overwrite: true });
      } else {
        scrambleEl.textContent = next;
      }
    }
    if (teleEl) {
      gsap.killTweensOf(teleEl);
      if (!motionOff) {
        ...
        gsap.to(teleEl, { duration: 0.4, scrambleText: { text: tele, chars: "ABCDEF0123456789", speed: 0.35 }, overwrite: true });
      }
    }
  }
  ...
  function onFrame() {
    ...
    hotspots.forEach((h) => {
      ...
      // Opacity via CSS .is-active only — avoid inline opacity fighting transitions
      if (!show) {
