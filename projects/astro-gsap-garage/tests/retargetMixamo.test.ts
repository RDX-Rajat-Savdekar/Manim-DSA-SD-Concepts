...
import { boneKey, aliasClipName, collectBoneMap, collectBindQuats, shouldDropClip } from "../src/lib/garageStage/retargetMixamo";
...
describe("retargetMixamo helpers", () => {
  ...
  describe("aliasClipName", () => {
    ...
    it("should resolve known Mixamo alias names to canonical names", () => {
      ...
      expect(aliasClipName("fightidle")).toBe("fight");
      expect(aliasClipName("Fight Idle")).toBe("fight");
    });

    it("should drop broken look / looking-around clips", () => {
      expect(shouldDropClip("look")).toBe(true);
      expect(shouldDropClip("Looking Around")).toBe(true);
      expect(shouldDropClip("idle")).toBe(false);
    });
  });
